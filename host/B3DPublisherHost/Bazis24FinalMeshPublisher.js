...
/* Compact only identical final vertex records. The key includes world position,
   BAZIS normal and BAZIS UV, so hard-normal and texture seams are preserved.
   Math.fround matches the Float32 payload that was already used by the viewer. */
function indexFinalVertices(positions, normals, texcoords) {
  const p = [], n = [], u = [], indices = [];
  const seen = new Map();
  const vertexCount = positions.length / 3;
  for (let vi = 0; vi < vertexCount; vi++) {
    const pi = vi * 3, ui = vi * 2;
    const values = [
      Math.fround(positions[pi]), Math.fround(positions[pi + 1]), Math.fround(positions[pi + 2]),
      Math.fround(normals[pi]), Math.fround(normals[pi + 1]), Math.fround(normals[pi + 2]),
      Math.fround(texcoords[ui]), Math.fround(texcoords[ui + 1])
    ];
    ...
    if (idx === undefined) {
      idx = p.length / 3;
      seen.set(key, idx);
      p.push(values[0], values[1], values[2]);
      n.push(values[3], values[4], values[5]);
      u.push(values[6], values[7]);
    }
    indices.push(idx);
  }
  return { positions: p, normals: n, texcoords: u, indices: indices };
}
...
function extractMesh(obj) {
  ...
  for (let si = 0; si < Number(mesh.TriListsCount); si++) {
    ...
    const indexed = indexFinalVertices(positions, normals, texcoords);
    sourceVertexCount += positions.length / 3;
    packedVertexCount += indexed.positions.length / 3;
    objectParts.push({
      objectId: objectId,
      name: objectName,
...
function indexBase64(values, bytes) {
  const b = Buffer.alloc(values.length * bytes);
  if (bytes === 2) {
...
const packed = parts.map(p => {
  const indexBytes = p.positions.length / 3 <= 65535 ? 2 : 4;
  return {
    objectId: p.objectId,
    name: p.name,
...
const payload = JSON.stringify({ format: 'local-view-bazis24-final-mesh-3', parts: packed, textures: textures }).replace(/<\//g, '<\\/');
const title = htmlEscape(modelName);
const savedPercent = sourceVertexCount ? Math.max(0, Math.round((1 - packedVertexCount / sourceVertexCount) * 100)) : 0;
const stats = detailCount + ' дет. · ' + triangleCount + ' треуг. · ' + edgeSegmentCount + ' рёбер · вершины −' + savedPercent + '%';
...
const isGL2=typeof WebGL2RenderingContext!=='undefined'&&gl instanceof WebGL2RenderingContext,uintExt=isGL2?true:gl.getExtension('OES_element_index_uint');
const u8=s=>{const q=atob(s),a=new Uint8Array(q.length);for(let i=0;i<q.length;i++)a[i]=q.charCodeAt(i);return a},f32=s=>{const a=u8(s);return new Float32Array(a.buffer,a.byteOffset,a.byteLength/4)},u16=s=>{const a=u8(s);return new Uint16Array(a.buffer,a.byteOffset,a.byteLength/2)},u32=s=>{const a=u8(s);return new Uint32Array(a.buffer,a.byteOffset,a.byteLength/4)};
...
function buf(data,target){const b=gl.createBuffer();gl.bindBuffer(target||gl.ARRAY_BUFFER,b);gl.bufferData(target||gl.ARRAY_BUFFER,data,gl.STATIC_DRAW);return b}
...
const parts=D.parts.map((m,pi)=>{const p=f32(m.p),n=f32(m.n),u=f32(m.u),e=f32(m.e),idx=m.it===2?u16(m.i):u32(m.i);if(m.it===4&&!uintExt)throw Error('Эта модель требует 32-битные индексы WebGL');return{...m,pickId:pi+1,p,n,u,e,idx,indexType:m.it===2?gl.UNSIGNED_SHORT:gl.UNSIGNED_INT,pb:buf(p),nb:buf(n),ub:buf(u),ib:buf(idx,gl.ELEMENT_ARRAY_BUFFER),eb:buf(e)}});
...
UI.dialogs.MessageBox('B3D Publisher: готово.\r\n\r\n' + outFile + '\r\n\r\n' + detailCount + ' деталей, ' + triangleCount + ' треугольников, ' + edgeSegmentCount + ' видимых рёбер. Вершины payload: ' + sourceVertexCount + ' → ' + packedVertexCount + ' (−' + savedPercent + '%).');